...
test({
  'parseFeedPage': {
    'returns filtered nr releases and the next olderThan on a happy-path response': () => {
      const feed = {
        stories: {
    ...
    'throws FeedShapeError (not TypeError) when feed.stories is missing': () => {
      assert.throws(
        () => parseFeedPage({}),
    ...
    'throws FeedShapeError when feed.stories is null': () => {
      assert.throws(() => parseFeedPage({ stories: null }), FeedShapeError)
    },

    'throws FeedShapeError when feed.stories.entries is not an array': () => {
    ...
    'does not raise TypeError before the typed error': () => {
      try {
  ...
  'isJsonContentType / assertJsonContentType': {
    'accepts plain application/json': () => {
      assert.strictEqual(isJsonContentType('application/json'), true)
    },
    'accepts application/json with charset parameter and casing variants': () => {
      assert.strictEqual(isJsonContentType('application/json; charset=utf-8'), true)
      assert.strictEqual(isJsonContentType('Application/JSON;charset=UTF-8'), true)
      assert.strictEqual(isJsonContentType('  application/json  '), true)
    },
    'rejects HTML and missing content-types': () => {
    ...
    'throws FeedShapeError from assertJsonContentType when the type is wrong': () => {
    ...
    'does not throw from assertJsonContentType when the type is JSON': () => {
  ...
  'parseFollowedArtistsPanel': {
    'returns normalised release objects on a happy-path fixture with two items': () => {
    ...
    'returns [] when the panel container is present but contains no new-release items': () => {
      const html = wrapPanel('')
      assert.deepStrictEqual(parseFollowedArtistsPanel(html), [])
    },
    'throws FeedShapeError when id="new-releases-vm" is absent (login redirect)': () => {
    ...
    'throws FeedShapeError on non-string input without raising TypeError': () => {
      try {
    ...
    'ingests CSS-hidden items past the "show more" cutoff': () => {
    ...
    'drops individual items whose data-item-json fails to parse but keeps the rest': () => {
    ...
    'does not match li elements that lack the new-release class even if they carry data-item-json': () => {
  ...
  'parsePagedataUsername': {
    ...
    'returns null on non-string input without throwing': () => {
      assert.strictEqual(parsePagedataUsername(null), null)
      assert.strictEqual(parsePagedataUsername(undefined), null)
    },
    'extracts a username from the captured logged-in feed page when present': () => {
    ...
    'finds the followed-artists panel sentinel in the captured logged-in feed page': () => {
      const fixturePath = path.resolve(__dirname, '../../../..', 'temp', 'feed.html')
      if (!fs.existsSync(fixturePath)) return
      const html = fs.readFileSync(fixturePath, 'utf8')
      const items = parseFollowedArtistsPanel(html)
  ...
  'usernameFromBandcampUrl': {
    'extracts the username segment from a profile URL': () => {
    ...
    'rejects reserved paths that are not usernames': () => {
    ...
    'rejects subdomain hosts and non-Bandcamp hosts': () => {
    ...
    'rejects bare-host URLs and bad input': () => {
  ...
  'isBandcampHostedUrl / partitionBandcampHosted': {
    'accepts bandcamp.com hosts and subdomains': () => {
    ...
    'rejects custom artist domains and non-Bandcamp hosts': () => {
    ...
    'partitions a release list into kept (bandcamp.com) and dropped (custom domains)': () => {
      const releases = [
    ...
    'returns empty arrays for non-array input': () => {
  ...
  'mergeReleases': {
    'dedupes on item_url across two sources, panel-first wins': () => {
      const panel = [
    ...
    'falls back to item_id when item_url is missing on one side': () => {
    ...
    'keeps disjoint entries from both lists': () => {
      const a = [{ item_id: 1, item_url: 'https://a/1' }]
      const b = [{ item_id: 2, item_url: 'https://a/2' }]
      assert.strictEqual(mergeReleases(a, b).length, 2)
    },
    'keeps entries that have neither item_url nor item_id (no key, no dedup)': () => {
      const a = [{ stray: true }]
      const b = [{ stray: true }]
      assert.strictEqual(mergeReleases(a, b).length, 2)
    },
    'ignores non-array inputs without throwing': () => {
  ...
  'releaseKey': {
    'prefers item_url over item_id': () => {
      assert.strictEqual(releaseKey({ item_url: 'https://a/x', item_id: 1 }), 'url:https://a/x')
    },
    'falls back to item_id when item_url is absent': () => {
      assert.strictEqual(releaseKey({ item_id: 7 }), 'id:7')
    },
    'returns null for non-objects and missing-key objects': () => {
