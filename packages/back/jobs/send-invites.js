...
const { renderInvite } = require('../services/email-templates')
...
const sendInviteEmail = async (email, inviteCode) => {
  ...
  const { subject, contentHtml, text, category } = renderInvite({ inviteUrl })
  await scheduleEmail(process.env.INVITE_EMAIL_SENDER, email, subject, text, contentHtml, category)
}
