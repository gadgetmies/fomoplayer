-- This should match 20210901072903-remove-nulls-from-preview-waveforms-up.sql
CREATE OR REPLACE FUNCTION track_details(track_ids integer[], meta_account_user_id_in integer)
    returns TABLE
            (
                track_id integer,
                title    text,
                heard    timestamp with time zone,
                duration integer,
...
WITH
   ...
   , authors AS (
    WITH unique_authors AS (
        SELECT
            ...
            artist_id,
            artist_name,
            coalesce(bool_or(meta_account_user_id = meta_account_user_id_in), false) AS following
        FROM
            limited_tracks lt
    ...
    SELECT
        track_id
         , json_agg(
            json_build_object('name', artist_name, 'id', artist_id, 'following', following)
            ORDER BY following DESC, artist_name
        ) AS authors
    FROM
        unique_authors
    GROUP BY 1
   ...
   , labels AS (
    ...
    SELECT
        track_id
         , json_agg(
            json_build_object('name', label_name, 'id', label_id, 'following', following)
            ORDER BY following DESC, label_name
        ) AS labels
    FROM unique_labels
    GROUP BY 1
)
