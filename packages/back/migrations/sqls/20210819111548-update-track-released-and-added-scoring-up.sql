...
SELECT track_id,
       GREATEST(0, 60 - DATE_PART('days', NOW() - LEAST(NOW(), MIN(store__track_released)))) AS score
FROM track
...
SELECT track_id, 60 - GREATEST(0, DATE_PART('days', NOW() - LEAST(NOW(), track_added))) AS score
FROM track;
