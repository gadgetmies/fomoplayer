...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .provider-note { color: #777; font-size: 0.75rem; margin-top: 1.5rem; }
  </style>
</head>
<body>
  ...
  router.get('/login/extension', (req, res) => {
    ...
    const requestedRedirectUri = req.query.redirect_uri ?? req.session?.extensionRedirectUri
  ...
  <div class="actions">
    ...
    return res.status(200).type('html').send(extensionPageShell('Extension Access',
      `<h1>Sign in to Fomo Player</h1>
  <p>The Fomo Player browser extension is requesting access to your account. Sign in to continue.</p>
  <div class="actions">
  ...
  <p class="provider-note">Fomo Player uses Google as its identity provider. The extension only ever stores Fomo Player tokens — it never sees your Google credentials.</p>`))
  })
  ...
  router.post('/extension/token', async (req, res, next) => {
    try {
      ...
      const result = consumeCodeFn(code, codeVerifier, { redirectUri: redirectUri || null })
      if (!result) {
        return res.status(401).json({ error: 'Invalid, expired, or already used authorization code' })
      }
