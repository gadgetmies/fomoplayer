# Bandcamp watch fetch

## Purpose

The scheduled Bandcamp watch job iterates each followed artist,
label, and playlist on each tick and refreshes their releases by
walking the artist/label page and fetching every release page it
exposes. Bandcamp's unofficial endpoints rate-limit aggressively,
and previously this job re-fetched every release on every tick —
even ones already in the database. This capability covers the
batch-size and freshness controls that bound how many followees
run per tick, the URL-skip filter that bounds how many release
pages a single followee fetches, the per-followee metrics
recorded on the operation `source` row, and the contract that
page-parse errors degrade gracefully without aborting the tick.

The implementation lives in
`packages/back/jobs/watches/shared/logic.js` (the per-store
fetch jobs and the shared rate-limit circuit-breaker),
`packages/back/jobs/watches/shared/db.js` (the followee-detail
queries and `updateSourceDetails`),
`packages/back/routes/stores/bandcamp/logic.js`
(`getArtistTracks` / `getLabelTracks` and the URL-skip filter),
and `packages/back/routes/stores/bandcamp/db.js`
(`queryKnownReleaseUrls`).
