...
A multi-target webpack watch entry-point for the browser extension
that rebuilds on save into each browser's `build/<browser>/`
directory, so contributors can iterate on extension UI without
re-running the production build per save.
