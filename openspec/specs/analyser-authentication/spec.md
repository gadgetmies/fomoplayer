...
Define how the analyser (Python worker) authenticates to the Fomo Player
backend and resolves its connection configuration: it uses a Fomo Player API
key (`Bearer fp_<id>`) on every request, reads the backend base URL from
`FOMOPLAYER_API_URL`, and contains none of the prior Google OIDC login
machinery.
