...
- `packages/back/config.js` — Reads `OIDC_HANDOFF_URL`, `OIDC_HANDOFF_SECRET`,
  `PREVIEW_ENV`, `GITHUB_ACTIONS_OIDC_REPO`, etc.
...
| Railway preview — consumer (`…-pr-N…`) | Consumer | Handoff flow via authority | Not the recommended target for CLI | **GitHub Actions OIDC** (`PREVIEW_ENV` + `GITHUB_ACTIONS_OIDC_REPO`) |
